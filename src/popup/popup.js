...
class PopupManager {
  ...
  async loadApiKey() {
    try {
      ...
      if (response.success && response.apiKey) {
        ...
        document.getElementById('apiKeyInput').value = this.maskApiKey(response.apiKey);
      }
    } catch (error) {
  ...
  async checkActiveTab() {
    try {
      ...
      const isValidDomain = tab.url.includes('x.com') || tab.url.includes('twitter.com');
      
      if (isValidDomain) {
        this.showNotification('Extension is active on this page', 'success');
      } else {
        this.showNotification('Visit X.com or Twitter.com to use this extension', 'warning');
      }
    } catch (error) {
